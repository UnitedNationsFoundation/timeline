<?php
    ...
    $timeline_video             =   get_field('timeline_video');
    $timeline_video_url         =   get_field('timeline_video_url');
    $addclass                   =   ( get_field('timeline_overlay') )    ?   ' no-header-overlay':'';
?>
<div class="timeline-item-post<?php echo $addclass; ?>">
    <div class="background__cont"
        <?php if($timeline_background_image) { ?>style="background-image: url(<?php echo $timeline_background_image['sizes']['background-size']; ?>);"
        <?php } elseif ($timeline_background_color) { ?>style="background-color: <?php echo $timeline_background_color; ?>;"<?php } ?>>
    ...
    <div class="post__cont centerDiv">
        <div class="text__cont__row centerDiv">
            <div class="video__cont">
                <?php if ($timeline_video) { ?>
                    <div class="video">
                        <video autoplay muted loop><source src="<?php echo $timeline_video; ?>" type="video/mp4"></video>
                    </div>
               <?php } elseif ($timeline_video_url) { ?>
                    <div class="video">
                        <?php echo wp_oembed_get( $timeline_video_url) ?>
                    </div>
               <?php } ?>     
            </div>
            <div class="text__cont--video"><?php echo $timeline_text; ?></div>
        </div>
    </div>
</div>